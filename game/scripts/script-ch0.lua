function ch0script()
	...
	if cl == 1 then
		...
		cw('???',"Hé !")
	elseif cl == 2 then  
		cw('bl', 'J\'aperçois, de loin, une fille chiante courir dans ma direction, agitant ses bras en l\'air comme si elle se fichait de l\'attention qu\'elle pourrait attirer.')
	elseif cl == 3 then
		cw('bl', 'Cette fille est Sayori, ma voisine et amie d\'enfance.')
	elseif cl == 4 then
		cw('bl',"Vous savez, le genre d'ami avec qui vous n'auriez jamais pensé traîner encore aujourd'hui, mais ça s'est fait comme ça, parce que vous vous connaissez depuis longtemps ?")
	elseif cl == 5 then
    ...
    elseif cl == 11 then
		cw('mc',"Peut-être, mais seulement parce que j'ai décidé de m'arrêt.")
	elseif cl == 12 then
		updateSayori('5c')
	...
	elseif cl == 10009 then 
		cw('s',"Ce n'est pas ça...")
	elseif cl == 10010 then 
		cw('s',"Non, ce n'est pas ça...")
	elseif cl == 10011 then 
		updateSayori('4','w')
		cw('s',"Qu'est-ce que c'est ?")
	elseif cl == 10012 then 
		cw('s',"Qu'est-ce que je suis ?")
	elseif cl == 10013 then 
		cw('s',"Faites en sorte que ça s'arrête !")
	elseif cl == 10014 then 
		cw('s',"S'IL VOUS PLAÎT ARRÊTEZ !")
	...
	if cl == 10016 then 
		bgUpdate('warning')
		cw('bl','Ceci est un accès non-officiel de Doki Doki Literature Club.')
	elseif cl == 10017 then
		cw('bl','Cet accès, ou le jeu original n\'est pas conseillé pour les enfants ou les personnes facilement perturbables.')
	elseif cl == 10018 then
		cw('bl','Les individus souffrant d\'anxiété ou de dépression n\'auront peut-être pas une bonne expérience en jouant à ce jeu. Pour les avertissements, merci de visiter : http://ddlc.moe/warning.html')
	elseif cl == 10019 then
		cw('bl','En jouant à Doki Doki Literature Club, vous acceptez que vous avez au moins 13 ans, et que vous vous attendrez à du contenu hautement perturbant.')
	elseif cl == 10020 then
		menutext = ''
		choices = {"J'accepte,'Je n'accepte pas."}
		choice_enable()
	elseif cl >= 10021 then
		if choicepick == "Je n'accepte pas." then
			game_quit()
		else
			cl = 10021
			bgUpdate('warning2')
