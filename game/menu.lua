...
function menu_enable(m)
	...
	if menu_type == 'mainyesno' then
		menutext = 'Êtes-vous sûr de retourner au menu principal ?\nVous allez perdre le progrès non-sauvegardé.'
		itemnames = {'Oui','Non'}
		
	elseif menu_type == 'quityesno' then
		menutext = 'Êtes-vous sûr de vouloir quitter le jeu ?'
		itemnames = {'Oui','Non'}
	...
	elseif menu_type == 'title' then
		...
		itemnames = {'Nouvelle partie','Charger','Paramètres','Aide','Quitter'}
		if persistent.ptr == 1 then itemnames[1] = glitchtext(10) end
		
	elseif menu_type == 'settings' then
		menutext = 'Paramètres'
		itemnames = {'Position du texte','Vitesse du texte','Avancement auto','Personnages','Sauvegarder'}
		
	elseif menu_type == 'settings2' then
		menutext = 'Paramètres'
		itemnames = {'Position du texte','Personnages','Sauvegarder'}
	
	elseif menu_type == 'characters' then
		menutext = 'Personnages'
		itemnames = {'Supprimer monika.chr','Supprimer natsuki.chr','Supprimer sayori.chr','Supprimer yuri.chr','Restaurer'}
	
	elseif menu_type == 'pause' or menu_type == 'pause2' then
		menutext = 'Menu pause'
		itemnames = {'Histoire','Sauvegarder','Charger','Menu principal','Paramètres','Aide','Quitter'}
	
	elseif menu_type == 'savegame' then
		menutext = 'Sauvegarder'
...
function menu_draw()
	...
	elseif menu_type == 'help' then
		...
		lg.print('A, L Trigger - Avancer le jeu,',16,45)
		lg.print('active les choix du menu',90,60)
		lg.print('B - Quitter le menu, Avancement auto On/Off',16,80)
		lg.print('X - (Menu) Page précédente, Skipping On/Off',16,100)
		lg.print('Y - (Menu) Page suivante, Menu du jeu',16,120)
		lg.print('Allez dans Paramètres > Personnages',16,150)
		lg.print('Supprimez tout dans ce repertoire :',16,180)
...
function menu_confirm()
	...
	if menu_type == 'title' then --title screen options
		...
		if m_selected == 2 then --new game
			if player == '' and global_os == 'Horizon' then --keyboard input for player name
				local input = {}
				input["type"] = "basic"
				input["hint"] = "Entrez le nom du joueur."
	...
	elseif menu_type == 'loadgame' then --load game confirm
		...
		if love.filesystem.isFile('save'..savenumber..'-'..persistent.ptr..'.sav') then
			...
			menutext = 'Le fichier de sauvegarde '..savenumber..' n\'existe pas'
		end
	...
	elseif menu_type == 'pause' or menu_type == 'pause2' then --pause menu options
		...
		elseif m_selected <= 6 and menu_type == 'pause2' then
			if m_selected == 3 and chapter == 30 then
				...
				menutext = "Il n'y a plus de raison de faire ça.\nNe t'inquiètes pas, je ne vais nul part."
			elseif m_selected == 6 then
